...
for (k in kernel_widths) {
  ...
  for (i in 1:n_folds) {
    ...
    for (j in colnames(train_data)) {
      mean_j = mean(train_data[,j,])
      sd_j = sd(train_data[,j,])
      train_data[,j,] = (train_data[,j,]-mean_j)/sd_j
      test_data[,j,] = (test_data[,j,]-mean_j)/sd_j
    }
...
```{r store_results}
exp_name = "exp_cli_50"
metrics = round(cbind(time, time_sd, mean_arcs, mean_arcs_sd, hamming, hamming_sd, pers_arcs, pers_arcs_sd, pers_arcs_total, pers_arcs_total_sd), digits=2)
to_export = list("mse"=round(mse,digits=2), "mse_sd" = round(mse_sd,digits=2), 
                 "mse_unfair" = round(mse_unfair,digits=2), "mse_unfair_sd" = round(mse_unfair_sd,digits=2),
                 "metrics" = metrics)
...
data_climate_i = reduce_resolution(data_climate_i, round(sqrt(length(data_climate_i[,1,1])*length(data_climate_i[,1,1])/25))  )
...
# First, make the folds
n_folds = 5
randomized_index = sample(1:length(data_climate[1,1,]))
split_index = split(randomized_index, cut(seq_along(randomized_index),n_folds,labels = FALSE))
folds = list()
for (i in 1:n_folds) {
  folds[[i]] = data_climate[,,split_index[[i]]]
}

# Select the interest kernel widths
kernel_width = 50 #(0.5,50,500,5000)
spatial_penalties = c(0.1,1,5,10,20,50,-1)
# Select the threshold for the global_graph
threshold = 0.95
# Select the number of points of partial inference
p_inference = 4
# Last time point
last_tp = length(data_climate[,1,1])-1

# Train and validate
mse = matrix(0, nrow = length(spatial_penalties), ncol = p_inference+1, dimnames = list(as.character(spatial_penalties), c("full", as.character(round(last_tp/p_inference*0:(p_inference-1)))) )  )
mse_sd = mse
mse_unfair = mse
mse_unfair_sd = mse
...
for (k in spatial_penalties) {
  ...
  for (i in 1:n_folds) {
    ...
    network = NULL
    if (k<0) {
      network = tvdbn::learn_tvdbn(train_data, kernel_bandwidth = kernel_width)
    }
    else {
    network = tvdbn::learn_tvdbn(train_data, spatial_penalty = k, kernel_bandwidth = kernel_width)
    }
    tf = Sys.time()
    # Store time
    time_k = c(time_k, tf-t0)
    ...
    for (j in 1:length(test_data[1,1,])) {
      mse_i = c()
      mse_i["full"] = tvdbn::inference(network, test_data[,,j],ini=0,end=length(data_climate[,1,1])-1)[[4]]
      for (sp in 1:p_inference-1) {
        mse_i[as.character(round(last_tp/5*sp))] = tvdbn::inference(network, test_data[,,j],ini=round(last_tp/5*sp),end=round(last_tp/5*(sp+1)))[[4]]
      }
      mse_k = rbind(mse_k,mse_i)
    ...
    for (j in 1:length(train_data[1,1,])) {
      mse_unfair_i = c()
      mse_unfair_i["full"] = tvdbn::inference(network, train_data[,,j],ini=0,end=length(data_climate[,1,1])-1)[[4]]
      for (sp in 1:p_inference-1) {
        mse_unfair_i[as.character(round(last_tp/5*sp))] = tvdbn::inference(network, train_data[,,j],ini=round(last_tp/5*sp),end=round(last_tp/5*(sp+1)))[[4]]
      }
      mse_unfair_k = rbind(mse_unfair_k, mse_unfair_i)
