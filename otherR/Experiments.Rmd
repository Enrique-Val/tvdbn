...
```{r load_data}
month_range <- c(10,12)     #period of months
...
```{r month_aggregate}
data_climate_pr = NCEP.aggregate(data_climate, DAYS = TRUE, HOURS = FALSE, fxn = "mean")
...
transform_to_dataset <- function(data_NCEP) {
  ...
  for (i in dimnames(data_NCEP)[[1]]) {
    for (j in dimnames(data_NCEP)[[2]]) {
      ...
      name <- paste("lon",i_aux,"lat",j_aux,"",sep="")
...
tvdbn_climate.fit = tvdbn::learn_tvdbn(climate, type = "autoregressive", max_parents = 5, kernel_bandwidth = (length(climate[,1])^2)/49*(1))
```
## Validación con motor dataset
```{r import_motor, include=FALSE}
tmp = read_csv("../data/motor_measures_v2_red.csv")

# Normalize using z-score
means = colMeans(tmp)
sds = sapply(tmp, mean, na.rm = TRUE)
...
library(abind)
# Truncate every serie to have only 150 time points and add them in an 3D array
mat_list = list()
...
for (k in kernel_widths) {
  ...
  for (i in 1:n_folds) {
    ...
    pers_arcs_k = c(pers_arcs_k, length(arcs(global_graph(sp1, threshold = threshold))[,1])/mean(tvdbn::n_arcs(network))*100)
    
    for (j in length(test_data[1,1,])) {
      mse = c(mse,tvdbn::inference(network, test_data[,,j],ini=0,end=length(array_3d[,1,1])-1)[[4]])
    }
    for (j in length(train_data[1,1,])) {
      mse_unfair = c(mse_unfair,tvdbn::inference(network, train_data[,,j],ini=0,end=length(array_3d[,1,1])-1)[[4]])
    }
  } # end for i in folds
  
  time = c(time, mean(time_k))
  hamming = c(hamming, mean(hamming_k))
  mean_arcs = c(mean_arcs, mean(mean_arcs_k))
  pers_arcs = c(pers_arcs, mean(pers_arcs_k))
  
  time_sd = c(time_sd, sd(time_k))
  hamming_sd = c(hamming_sd, sd(hamming_k))
  mean_arcs_sd = c(mean_arcs_sd, sd(mean_arcs_k))
  pers_arcs_sd = c(pers_arcs_sd, sd(pers_arcs_k))
